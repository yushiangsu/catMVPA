function Searchlight_Mat = get_searchlight_mat(radius, mask_file, ref_file)
...
%% Configuration
if nargin == 3
    fprintf('%s\n', 'Reference image were included, generated a temporary mask file.')
...
for i = 1:length(mask_Idx)
    ...
    if find(Searchlight_corr(:,1) < 1 | Searchlight_corr(:,1) > size(mask_Vol, 1) | ...
        ...
        Searchlight_Mat(i, :) = sub2ind(size(mask_Vol), ...
